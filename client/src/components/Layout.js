...
const Layout = () => {
  ...
  const handleInputSubmit = async (e) => {
    ...
    setResponseText((prev) => [...prev, { user: input, bot: <LoadingDots /> }]);
      ...
      // Caso 1: Risposta relativa ai bidoni
      if (chatbotResponse.type === "bidoni") {
        const { data } = chatbotResponse;
        console.log(chatbotResponse);
        const responseText  = chatbotResponse.results[0].generated_text;
        if (map) {
          data.forEach((bidone) => {
            const [lat, lng] = JSON.parse(bidone.coord);
            L.marker([lat, lng])
              .addTo(map)
              .bindPopup(`Temperatura: ${bidone.temperatura}°<br> Riempimento: ${bidone.riempimento}%`)
              .openPopup();
          });
        }
        ...
        setResponseText((prev) => {
          const updated = [...prev];
          updated[updated.length - 1] = { user: input, bot: responseText };
          return updated;
        });
      }else{
        console.log('CHAT');
